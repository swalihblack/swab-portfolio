...
export function BlogPostView({ post }: BlogPostViewProps) {
  ...
  return (
    <article>
      <Link to="/blog" className="inline-flex items-center gap-2 font-body text-sm text-accent hover:text-accent/80 mb-8 transition-colors">
        <ArrowLeft size={16} />
        {t('blog.backToBlog')}
      </Link>
      {post.coverUrl && (
        <div className="rounded-lg overflow-hidden mb-8 bg-muted">
          <img src={post.coverUrl} alt={post.title} className="w-full h-auto max-h-96 object-cover" />
        </div>
      )}
      ...
      {post.tags.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap mt-10 pt-6 border-t border-border">
          ...
          {post.tags.map((tag) => (
            <span key={tag} className="rounded-sm bg-secondary px-2 py-0.5 font-body text-xs text-secondary-foreground">{tag}</span>
          ))}
        </div>
      )}
    </article>
